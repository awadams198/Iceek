...
from app.models import Arena, Image, User, Review, db, review
...
@arena_routes.route('/')
def view_arenas():
    arenas = (Arena.query.join(User, User.id == Arena.userId).add_columns(User.username)
    .order_by(desc(Arena.created_at)).all())
...
def one_arena(id):
    ...
    oneArena['user'] = [singleArena.to_dict() for singleArena in user]
